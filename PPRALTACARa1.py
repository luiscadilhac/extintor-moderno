...
class PPRAApp:
    def __init__(self, root):
        ...
        self.cor_fonte = "#ffd700"  # Cor dourada para rótulos
        self.cor_texto = "#ff4500"  # Cor laranja para texto das caixas
        ...
        self.style.configure("TEntry", font=self.fonte_padrao, foreground=self.cor_texto)
        self.style.configure("TCombobox", font=self.fonte_padrao, foreground=self.cor_texto)
        self.style.configure("TFrame", background="#66cdaa")  # Cor de fundo turquesa
        self.style.configure("TLabelframe", font=("Verdana", 12, "bold"), foreground=self.cor_fonte, background="#66cdaa")
        self.style.configure("TLabelframe.Label", font=("Verdana", 12, "bold"), foreground=self.cor_fonte, background="#66cdaa")
        self.style.configure("Treeview", font=self.fonte_padrao, foreground=self.cor_fonte, background="#66cdaa")
        self.style.configure("Treeview.Heading", font=("Verdana", 12, "bold"), foreground=self.cor_fonte, background="#66cdaa")
        self.style.configure("TNotebook", font=self.fonte_padrao, background="#66cdaa")
        self.style.configure("TNotebook.Tab", font=self.fonte_padrao, background="#66cdaa")
    ...
    def create_scrollable_interface(self):
        ...
        # Criar canvas
        self.canvas = tk.Canvas(self.main_frame, bg="#66cdaa")
    ...
    def create_empresa_widgets(self):
        ...
        empresa_main_frame = ttk.Frame(self.cadastro_frame)
        ...
        ttk.Label(empresa_view_frame, text="ID:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=2)
        self.empresa_id_view = ttk.Entry(empresa_view_frame, width=10, font=self.fonte_padrao)
        self.empresa_id_view.grid(row=0, column=1, padx=5, pady=2)
        self.empresa_id_view.config(state='readonly')
        
        ttk.Label(empresa_view_frame, text="Nome:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        self.empresa_nome_view = ttk.Entry(empresa_view_frame, width=50, font=self.fonte_padrao)
        self.empresa_nome_view.grid(row=1, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W+tk.E)
        self.empresa_nome_view.config(state='readonly')
        
        ttk.Label(empresa_view_frame, text="CNPJ:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        self.empresa_cnpj_view = ttk.Entry(empresa_view_frame, width=20, font=self.fonte_padrao)
        self.empresa_cnpj_view.grid(row=2, column=1, padx=5, pady=2, sticky=tk.W)
        self.empresa_cnpj_view.config(state='readonly')
        
        ttk.Label(empresa_view_frame, text="Endereço:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=2)
        self.empresa_endereco_view = ttk.Entry(empresa_view_frame, width=50, font=self.fonte_padrao)
        self.empresa_endereco_view.grid(row=3, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W+tk.E)
        self.empresa_endereco_view.config(state='readonly')
    
    def create_setor_widgets(self):
        # Frame principal para Setor
        setor_main_frame = ttk.Frame(self.cadastro_frame)
        setor_main_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Frame para formulário de Setor
        setor_frame = ttk.LabelFrame(setor_main_frame, text="Setor")
        setor_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Frame para visualização de Setor
        setor_view_frame = ttk.LabelFrame(setor_main_frame, text="Visualização")
        setor_view_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        # Campos de visualização
        ttk.Label(setor_view_frame, text="ID:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=2)
        self.setor_id_view = ttk.Entry(setor_view_frame, width=10, font=self.fonte_padrao)
        self.setor_id_view.grid(row=0, column=1, padx=5, pady=2)
        self.setor_id_view.config(state='readonly')
        
        ttk.Label(setor_view_frame, text="Empresa:").grid(row=0, column=2, sticky=tk.W, padx=5, pady=2)
        self.setor_empresa_view = ttk.Entry(setor_view_frame, width=30, font=self.fonte_padrao)
        self.setor_empresa_view.grid(row=0, column=3, padx=5, pady=2, sticky=tk.W)
        self.setor_empresa_view.config(state='readonly')
        
        ttk.Label(setor_view_frame, text="Nome:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        self.setor_nome_view = ttk.Entry(setor_view_frame, width=50, font=self.fonte_padrao)
        self.setor_nome_view.grid(row=1, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W+tk.E)
        self.setor_nome_view.config(state='readonly')
        
        ttk.Label(setor_view_frame, text="Descrição:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        self.setor_descricao_view = ttk.Entry(setor_view_frame, width=50, font=self.fonte_padrao)
        self.setor_descricao_view.grid(row=2, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W+tk.E)
        self.setor_descricao_view.config(state='readonly')
    
    def create_cargo_widgets(self):
        # Frame principal para Cargo
        cargo_main_frame = ttk.Frame(self.cadastro_frame)
        cargo_main_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Frame para formulário de Cargo
        cargo_frame = ttk.LabelFrame(cargo_main_frame, text="Cargo")
        cargo_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Frame para visualização de Cargo
        cargo_view_frame = ttk.LabelFrame(cargo_main_frame, text="Visualização")
        cargo_view_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        # Campos de visualização
        ttk.Label(cargo_view_frame, text="ID:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=2)
        self.cargo_id_view = ttk.Entry(cargo_view_frame, width=10, font=self.fonte_padrao)
        self.cargo_id_view.grid(row=0, column=1, padx=5, pady=2)
        self.cargo_id_view.config(state='readonly')
        
        ttk.Label(cargo_view_frame, text="Setor:").grid(row=0, column=2, sticky=tk.W, padx=5, pady=2)
        self.cargo_setor_view = ttk.Entry(cargo_view_frame, width=30, font=self.fonte_padrao)
        self.cargo_setor_view.grid(row=0, column=3, padx=5, pady=2, sticky=tk.W)
        self.cargo_setor_view.config(state='readonly')
        
        ttk.Label(cargo_view_frame, text="Nome:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        self.cargo_nome_view = ttk.Entry(cargo_view_frame, width=50, font=self.fonte_padrao)
        self.cargo_nome_view.grid(row=1, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W+tk.E)
        self.cargo_nome_view.config(state='readonly')
        
        ttk.Label(cargo_view_frame, text="Descrição:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        self.cargo_descricao_view = ttk.Entry(cargo_view_frame, width=50, font=self.fonte_padrao)
        self.cargo_descricao_view.grid(row=2, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W+tk.E)
        self.cargo_descricao_view.config(state='readonly')
    
    def create_atividade_widgets(self):
        # Frame principal para Atividade
        atividade_main_frame = ttk.Frame(self.cadastro_frame)
        atividade_main_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Frame para formulário de Atividade
        atividade_frame = ttk.LabelFrame(atividade_main_frame, text="Atividade")
        atividade_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Frame para visualização de Atividade
        atividade_view_frame = ttk.LabelFrame(atividade_main_frame, text="Visualização")
        atividade_view_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        # Campos de visualização
        ttk.Label(atividade_view_frame, text="ID:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=2)
        self.atividade_id_view = ttk.Entry(atividade_view_frame, width=10, font=self.fonte_padrao)
        self.atividade_id_view.grid(row=0, column=1, padx=5, pady=2)
        self.atividade_id_view.config(state='readonly')
        
        ttk.Label(atividade_view_frame, text="Cargo:").grid(row=0, column=2, sticky=tk.W, padx=5, pady=2)
        self.atividade_cargo_view = ttk.Entry(atividade_view_frame, width=30, font=self.fonte_padrao)
        self.atividade_cargo_view.grid(row=0, column=3, padx=5, pady=2, sticky=tk.W)
        self.atividade_cargo_view.config(state='readonly')
        
        ttk.Label(atividade_view_frame, text="Descrição:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        self.atividade_descricao_view = ttk.Entry(atividade_view_frame, width=50, font=self.fonte_padrao)
        self.atividade_descricao_view.grid(row=1, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W+tk.E)
        self.atividade_descricao_view.config(state='readonly')
    
    def create_ambiente_widgets(self):
        # Frame principal para Ambiente
        ambiente_main_frame = ttk.Frame(self.cadastro_frame)
        ambiente_main_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Frame para formulário de Ambiente
        ambiente_frame = ttk.LabelFrame(ambiente_main_frame, text="Ambiente Laborativo")
        ambiente_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Frame para visualização de Ambiente
        ambiente_view_frame = ttk.LabelFrame(ambiente_main_frame, text="Visualização")
        ambiente_view_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        # Campos de visualização
        ttk.Label(ambiente_view_frame, text="ID:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=2)
        self.ambiente_id_view = ttk.Entry(ambiente_view_frame, width=10, font=self.fonte_padrao)
        self.ambiente_id_view.grid(row=0, column=1, padx=5, pady=2)
        self.ambiente_id_view.config(state='readonly')
        
        ttk.Label(ambiente_view_frame, text="Cargo:").grid(row=0, column=2, sticky=tk.W, padx=5, pady=2)
        self.ambiente_cargo_view = ttk.Entry(ambiente_view_frame, width=30, font=self.fonte_padrao)
        self.ambiente_cargo_view.grid(row=0, column=3, padx=5, pady=2, sticky=tk.W)
        self.ambiente_cargo_view.config(state='readonly')
        
        ttk.Label(ambiente_view_frame, text="Descrição:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        self.ambiente_descricao_view = ttk.Entry(ambiente_view_frame, width=50, font=self.fonte_padrao)
        self.ambiente_descricao_view.grid(row=1, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W+tk.E)
        self.ambiente_descricao_view.config(state='readonly')
    ...
    def create_relatorio_widgets(self):
        ...
        self.relatorio_canvas = tk.Canvas(relatorio_text_frame, bg="#66cdaa")
        ...
        self.relatorio_canvas.config(bg="#66cdaa")
    ...
    def create_ppra_widgets(self):
        ...
        self.ppra_canvas = tk.Canvas(ppra_text_frame, bg="#66cdaa")
        ...
        # Configurar a cor de fundo do canvas
        self.ppra_canvas.config(bg="#66cdaa")
    ...
    def show_empresa(self, index):
        ...
        # Atualizar campos de visualização
        self.empresa_id_view.config(state='normal')
        self.empresa_id_view.delete(0, tk.END)
        self.empresa_id_view.insert(0, str(empresa[0]))
        self.empresa_id_view.config(state='readonly')
        
        self.empresa_nome_view.delete(0, tk.END)
        self.empresa_nome_view.insert(0, empresa[1])
        
        self.empresa_cnpj_view.delete(0, tk.END)
        self.empresa_cnpj_view.insert(0, empresa[2])
        
        self.empresa_endereco_view.delete(0, tk.END)
        self.empresa_endereco_view.insert(0, empresa[3])
    ...
    def update_setores_empresa(self, empresa_id):
        ...
        if setores_empresa:
            ...
            self.setor_id_view.config(state='normal')
            self.setor_id_view.delete(0, tk.END)
            self.setor_id_view.config(state='readonly')
            
            self.setor_empresa_view.delete(0, tk.END)
            self.setor_nome_view.delete(0, tk.END)
            self.setor_descricao_view.delete(0, tk.END)
    ...
    def show_setor(self, index):
        ...
        self.setor_id_view.config(state='normal')
        self.setor_id_view.delete(0, tk.END)
        self.setor_id_view.insert(0, str(setor[0]))
        self.setor_id_view.config(state='readonly')
        
        self.setor_empresa_view.delete(0, tk.END)
        self.setor_empresa_view.insert(0, empresa_nome)
        
        self.setor_nome_view.delete(0, tk.END)
        self.setor_nome_view.insert(0, setor[1])
        
        self.setor_descricao_view.delete(0, tk.END)
        self.setor_descricao_view.insert(0, setor[2])
    ...
    def update_cargos_setor(self, setor_id):
        ...
        if cargos_setor:
            ...
            # Limpar campos de visualização
            self.cargo_id_view.config(state='normal')
            self.cargo_id_view.delete(0, tk.END)
            self.cargo_id_view.config(state='readonly')
            
            self.cargo_setor_view.delete(0, tk.END)
            self.cargo_nome_view.delete(0, tk.END)
            self.cargo_descricao_view.delete(0, tk.END)
    ...
    def show_cargo(self, index):
        ...
        self.cargo_id_view.config(state='normal')
        self.cargo_id_view.delete(0, tk.END)
        self.cargo_id_view.insert(0, str(cargo[0]))
        self.cargo_id_view.config(state='readonly')
        
        self.cargo_setor_view.delete(0, tk.END)
        self.cargo_setor_view.insert(0, setor_nome)
        
        self.cargo_nome_view.delete(0, tk.END)
        self.cargo_nome_view.insert(0, cargo[1])
        
        self.cargo_descricao_view.delete(0, tk.END)
        self.cargo_descricao_view.insert(0, cargo[2])
    ...
    def update_atividades_cargo(self, cargo_id):
        ...
        if atividades_cargo:
            ...
            # Limpar campos de visualização
            self.atividade_id_view.config(state='normal')
            self.atividade_id_view.delete(0, tk.END)
            self.atividade_id_view.config(state='readonly')
            
            self.atividade_cargo_view.delete(0, tk.END)
            self.atividade_descricao_view.delete(0, tk.END)
    ...
    def update_ambientes_cargo(self, cargo_id):
        ...
        if ambientes_cargo:
            ...
            # Limpar campos de visualização
            self.ambiente_id_view.config(state='normal')
            self.ambiente_id_view.delete(0, tk.END)
            self.ambiente_id_view.config(state='readonly')
            
            self.ambiente_cargo_view.delete(0, tk.END)
            self.ambiente_descricao_view.delete(0, tk.END)
    ...
    def show_atividade(self, index):
        ...
        self.atividade_id_view.config(state='normal')
        self.atividade_id_view.delete(0, tk.END)
        self.atividade_id_view.insert(0, str(atividade[0]))
        self.atividade_id_view.config(state='readonly')
        
        self.atividade_cargo_view.delete(0, tk.END)
        self.atividade_cargo_view.insert(0, cargo_nome)
        
        self.atividade_descricao_view.delete(0, tk.END)
        self.atividade_descricao_view.insert(0, atividade[1])
    ...
    def show_ambiente(self, index):
        ...
        self.ambiente_id_view.config(state='normal')
        self.ambiente_id_view.delete(0, tk.END)
        self.ambiente_id_view.insert(0, str(ambiente[0]))
        self.ambiente_id_view.config(state='readonly')
        
        self.ambiente_cargo_view.delete(0, tk.END)
        self.ambiente_cargo_view.insert(0, cargo_nome)
        
        self.ambiente_descricao_view.delete(0, tk.END)
        self.ambiente_descricao_view.insert(0, ambiente[1])
