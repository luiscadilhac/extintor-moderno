...
class MapaRiscosCIPA:
    ...
    def carregar_layout(self):
        ...
        if file_path:
            try:
                ...
                messagebox.showerror("Erro ao Carregar Layout", f"Não foi possível carregar o layout. Verifique se o arquivo é uma imagem válida e tente novamente.\nDetalhes do erro: {str(e)}")
    ...
    def abrir_form_risco(self, x=0, y=0, risco_index=None):
        ...
        def salvar():
            ...
            intensidade_selecionada = intensidade_var.get()
            if intensidade_selecionada not in [info["label"] for info in self.intensidades.values()]:
                messagebox.showerror("Erro", "Intensidade selecionada inválida!")
                return
    ...
    def _truncate_string(self, text, length):
        if len(text) <= length:
            return text
    ...
    def atualizar_lista_riscos(self):
        ...
        for i, risco in enumerate(self.riscos):
            ...
            self.tree.insert('', 'end', text=str(i + 1),
                           values=(tipo_label, self._truncate_string(risco["descricao"], 30), intensidade_label))
    ...
    def salvar_projeto(self):
        ...
        if file_path:
            try:
                ...
                messagebox.showerror("Erro ao Salvar Projeto", f"Não foi possível salvar o projeto. Verifique as permissões de escrita ou o caminho do arquivo.\nDetalhes do erro: {str(e)}")
    ...
    def carregar_projeto(self):
        ...
        if file_path:
            try:
                ...
                messagebox.showerror("Erro ao Carregar Projeto", f"Não foi possível carregar o projeto. Verifique se o arquivo JSON está válido.\nDetalhes do erro: {str(e)}")
    ...
    def exportar_mapa(self):
        ...
        if file_path:
            try:
                ...
                messagebox.showerror("Erro ao Exportar Mapa", f"Não foi possível exportar o mapa. Verifique as permissões de escrita ou o caminho do arquivo.\nDetalhes do erro: {str(e)}")
    ...
    def gerar_relatorio(self):
        ...
        if file_path:
            try:
                ...
                messagebox.showerror("Erro ao Gerar Relatório", f"Não foi possível gerar o relatório. Verifique as permissões de escrita ou o caminho do arquivo.\nDetalhes do erro: {str(e)}")
